...
class AsistenciaController extends AweController {
    ...
    public function accessRules() {
        return array(
            ...
            array('allow',
                'actions' => array('imprimir', 'anular', 'salir_sin_lector', 'registrar_sin_lector', 'salir', 'registra_y_actualiza_codigo_barra',
                    'registrar_sin_enter',
                    'registrar', '', 'update', 'admin', 'delete'),
      ...
      ');
            if (isset($modelos_socio)) {
                ...
                if ($bandera == 0) { // Ahun no tiene asistencia a esta reunión
                    ...
                    //   $mensajeConfirmandoRegistro=''.$genero.$mod->APELLIDO;
                    ...
                    if ($bandera_confirma == 0) {
                        if ($model->save()) {
                            $mensajeConfirmandoRegistro = $genero . $mod->APELLIDO . ' se registro su asistencia.';
                            Yii::app()->user->setFlash('success', $mensajeConfirmandoRegistro);
                            Yii::app()->getSession()->add('ultimo_registrado', $mensajeConfirmandoRegistro);
                            $socio['MENSAJE'] = $mensajeConfirmandoRegistro;
                            // $mensajeConfirmandoRegistro=$genero.$mod->APELLIDO.' se registro su asistencia.';
                            //   Yii::app()->user->setFlash('success',$mensajeConfirmandoRegistro);
                            // Yii::app()->getSession()->add('ultimo_registrado', $mensajeConfirmandoRegistro);
                     ...
                     $mensajeConfirmandoRegistro = 'El socio ' . $genero . $mod->APELLIDO . ' ya esta registrado en esta reunión';
                    Yii::app()->user->setFlash('success', $mensajeConfirmandoRegistro);
                    Yii::app()->getSession()->add('ultimo_registrado', $mensajeConfirmandoRegistro);
                    $socio['MENSAJE'] = $mensajeConfirmandoRegistro;
...
////Ya tiene asistencia a la reunion
    ...
    public function actionRegistrar($id) {
        ...
        if (isset($_POST['Asistencia'])) {
            ...
            $model_socio = Socio::model()->findByPk($model->CODIGO_SOCIO);
            if (isset($model_socio) and $model_socio->CODIGO > 0) {
                if ($model_socio->GENERO == 'F') {
                    $genero = " Sra/Srta. ";
                } else {
                    $genero = " Sr. ";
                ...
                if ($bandera == 0) { // Ahun no tiene asistencia a esta reunión
                    ...
                    if ($model->save()) {
                        ...
                        //$this->redirect(array('imprimir', 'id' => $modelo_asistencia->CODIGO_ASISTENCIA)); // nO SE IMPRIMIR 
                        $this->redirect(array('registrar', 'id' => $id));
                    }
                } // fin de Ahun no tiene asistencia
                else {
    ...
    public function actionRegistrar_sin_enter($id) {
        //BUSCAR LA REUNION
        ...
        if (isset($_POST['Asistencia'])) {
            ...
            /*   $mensajeConfirmandoRegistro = '' . $genero . $model_socio->APELLIDO;
              ...
              if ($model->save()) {
              //   $this->redirect(array('imprimir', 'id' => $model->CODIGO_ASISTENCIA));
              // $this->redirect(array('registrar_sin_enter', 'id' => $id));
              }
             */
        ...
        $this->render('registrar_sin_enter', array(
            'model' => $model,
            'model_reunion' => $model_reunion,
        ));
    }
...
// Proceso de imprimir
        ...
        $recorriendo_y = 0;
        $barcode = printer_create_font("Arial", 72, 48, 500, false, false, false, 0);
        $arial = printer_create_font("Arial", 148, 76, PRINTER_FW_MEDIUM, false, false, false, 0);
        printer_select_font($enlace, $arial);
        //   printer_delete_font($font);
        // printer_draw_text($enlace,"     Ticket Nº: ",0, 0); 

        printer_draw_text($enlace, "     ''JAAPA SAN VICENTE DE LACAS''", 0, 0);
        printer_draw_text($enlace, "  " . $model->cODIGOREUNION->cODIGOTIPO->TIPO, 0, 150);

        printer_select_font($enlace, $barcode);
        printer_draw_text($enlace, utf8_decode('' . $fecha_formateada), 0, 320);


        printer_select_font($enlace, $barcode);
        //printer_draw_text($enlace, utf8_decode("Recibo N' : ".$rec->ID_FACTURA), 0, 300);
        printer_draw_text($enlace, utf8_decode('NOMBRES: ' . $model_socio->APELLIDO), 0, 470);
        printer_draw_text($enlace, utf8_decode('CI/RUC : ' . $model_socio->CI . '        GRUPO:  ' . $model_socio->grupo->GRUPO), 0, 620);
        //  $recibo->CAMBIO = number_format('VALOR',0,'.','');        


        $total = 0.00;
        $mm_inicia_detalle = 1200; //Inica a imprimir los detalles en el eje y                  
        ...
        $mm_inicia_detalle = $mm_inicia_detalle + 600;
        ...
        if (isset($_POST['Asistencia'])) {
            ...
            if ($model->CODIGO_SOCIO > 0) { // Existe valor en CODIGO DE SOCIO
                ...
                if ($bandera == 0) { // Ahun no tiene asistencia a esta reunión
                    ...
                    $mensajeConfirmandoRegistro = 'El socio ' . $genero . $model_socio->APELLIDO . ' ya esta registrado en esta reunión, y fue emitido el Ticket';
                    Yii::app()->user->setFlash('success', $mensajeConfirmandoRegistro);
                    $this->redirect(array('registrar_sin_lector', 'id' => $id));
                    //   $this->redirect(array('imprimir', 'id' => $modelo_asistencia->CODIGO_ASISTENCIA)); // nO SE IMPRIMIR 
                }
