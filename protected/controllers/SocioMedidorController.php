...
//                    $columna['APELLIDO'],
    ...
    public function actionBuscar_socio_entrega_documento() {
        ...
        if (!isset($_GET['ajax'])) {
            ...
            if ($_POST['socio'] == "38" || $_POST['socio'] == "34" || $_POST['socio'] == "65" || $_POST['socio'] == "74" || $_POST['socio'] == "105" || $_POST['socio'] == "96" || $_POST['socio'] == "154" || $_POST['socio'] == "150" || $_POST['socio'] == "234" || $_POST['socio'] == "208" || $_POST['socio'] == "235" || $_POST['socio'] == "239" || $_POST['socio'] == "252" || $_POST['socio'] == "257" || $_POST['socio'] == "273" || $_POST['socio'] == "275") {
                $datos = $datos;
            } else {
                $datos = null;
            }
 ...
 ORDER BY d.`FECHA_COBRO` ASC
    ...
    public function actionCorte() {
        $model_socios = Socio::model()->findAllBySql('
                   SELECT 
                   sm.ID AS CODIGO,
        sm.`ID` AS COD_BARRA, -- Número de socio medidor del deudor        
        s.APELLIDO, s.CI, s.CELULAR, s.TELEFONO, 
	m.NUMERO AS OBS, m.ORDEN_RECORIDO AS COD_GRUPO,    
        sm.`INACTIVO` AS ESTADO, 
       (SELECT COUNT(DISTINCT fac.ID)
                   FROM detalle AS det 
                   INNER JOIN rubro AS rub
		   ON det.`ID_RUBRO` = rub.ID
                   ...
                   AND det.ESTADO=0
                   AND fac.TIPO = 1
                    AND (rub.`DESCRIPCION` LIKE "CONSUMO DE AGUA POTABLE%" OR rub.`DESCRIPCION` LIKE "ALCANTARILLADO%")
                    ) AS COD_USUARIO,                     
...
f.`ESTADO` = 0
AND f.`TIPO` = 1
AND (r.`DESCRIPCION` LIKE "CONSUMO DE AGUA POTABLE%" OR r.`DESCRIPCION` LIKE "ALCANTARILLADO%")
AND s.ESTADO = 1
-- AND sm.`INACTIVO` = 0
GROUP BY sm.ID
ORDER BY COD_USUARIO DESC, FOTO DESC
        ...
        foreach ($model_socios as $modelo_socio) {
            ...
            $modelo_socio->COD_USUARIO == 3 // Deudas iguales a 3 meses
                ...
                $sql = 'select buscar_fecha_ultima_factura_deuda(' . $modelo_socio->CODIGO . ') AS FECHA_NACIMIENTO';
                 $modelo_primera_factura_deuda = Socio::model()->findBySql($sql);                     
                        ...
                        // &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&     
                        // FAlta controlar los meses ahun en deuda para no volver a cargar
...
//                                   $command->queryScalar();
                        ...
                        // &&&&&&&&&&& AGREGAR DEUDA SEMESTRAL &&&&&&&&&&&&&&
                        // &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&     
                        // FAlta controlar los meses ahun en deuda para no volver a cargar
//                                   $query = 'call aplicar_sancion_semestral("'.$modelo_primera_factura_deuda->FECHA_NACIMIENTO .'",'.$modelo_ultima_factura_deuda->ID.')';
//                                   $command = Yii::app()->db->createCommand($query);
//                                   $command->queryScalar();
                        // &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
                        // &&&&&&&& FIN DE AGREGAR DEUDA TRIMESTRAL &&&&&&&&&&
    ...
    public function actionCorte_oficios_word() {
        $model_socios = Socio::model()->findAllBySql('
                   SELECT s.APELLIDO, s.CI, s.CELULAR, s.TELEFONO, s.COD_GRUPO,
	m.NUMERO AS OBS,
...
ORDER BY COD_USUARIO DESC, FOTO DESC
                        ');
        $contenido = $this->renderPartial("corte_oficios_word", array('model_socios' => $model_socios), true);
        Yii::app()->request->sendFile("Corte de servicio de agua potable al " . gmdate('d_M_Y') . ".doc", $contenido);
        $this->render('corte_oficios_word', array(
            'model_socios' => $model_socios,
        ));
    }
        ...
        for ($i = 1; $i <= 2; $i++) {
            ...
            foreach ($detalles as $det) {
                if (($det['TIPO']) == 1) { //FACTURA 
                    ...
                    $mora_2021 = strpos($det['DESCRIPCION'], 'MORA SERVICIO DE AGUA POTABLE'); //posicion 0
                    $mora_alcantarillado_2021 = strpos($det['DESCRIPCION'], 'MORA SERVICIO ALCANTARILLADO'); //posicion 0
                    $posicion_alcantarillado = strpos($det['DESCRIPCION'], 'ALCANTARILLADO'); //posicion debe ser 0
                    if ($posicion !== 0
                            and $basico !== 0
                            and $posicion_alcantarillado !== 0
                            and $mora !== 0
                            and $mora_2021 !== 0
                            and $mora_alcantarillado_2021 !== 0
                    ) { //NO ES CONSUMO DE AGUA POTABLE NI ALCANTARILLADO    //and ($mora !== 0 or $mora_2021 !== 0)                        
                    ...
                    if ($posicion === 0) {
                        ...
                        $mora_a_comparar_2021 = 'MORA SERVICIO DE AGUA POTABLE ' . $descripcion_final;
                        foreach ($detalles as $det_mora) {
                            if (($det_mora['TIPO']) == 1) { //FACTURA 
                                if ($det_mora['DESCRIPCION'] == $mora_a_comparar or $det_mora['DESCRIPCION'] == $mora_a_comparar_2021) {
                                    $mora = $mora + $det_mora['V_TOTAL'];
                                }
                            }
                        }
                    ...
                    if ($posicion_alcantarillado === 0) {
                        ...
                        $mora = 0;
                        //Buscamos mora                        
                        $mora_a_comparar_2021 = 'MORA SERVICIO DE ALCANTARILLADO ' . $descripcion_final;
                        foreach ($detalles as $det_mora) {
                            if (($det_mora['TIPO']) == 1) { //FACTURA 
                                if ($det_mora['DESCRIPCION'] == $mora_a_comparar_2021) {
