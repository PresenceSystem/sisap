 ...
 */
//$this->menu = array(
//    array('label' => Yii::t('AweCrud.app', 'Convertir a excel'), 'icon' => 'pencil', 'url' => array('lista_excel_')),
...
<div class="span12">
    ...
    <h3 class="btn-primary text-center ">
        SOCIOS ACTIVOS DEL ALCANTARILLADO <?= $model_grupo->DESCRIPCION ?>
    </h3>
...
//                    ' AND INACTIVO = 0');
            ...
            $model_acometida_alcantarillado = AcometidaAlcantarillado::model()->findAllBySql(
                    'select a.* from acometida_alcantarillado as a
                     ...
                     where sm.CODIGO_SOCIO = ' . $modelo_socio->CODIGO
                    . ' and a.INACTIVO = 0');
            //.' and a.ID_GRUPO = '.$model_grupo->COD_GRUPO);
            //***************************************************************
...
<div class="panel panel-primary row span span12">
    <div class="panel-heading">Resumen de las instalaciones de alcantarillado <?= $model_grupo->DESCRIPCION ?></div>
...
WHERE `INACTIVO` = 0 AND ID_GRUPO = '.$model_grupo->COD_GRUPO.'
GROUP BY `ESTADO`');
        ?>        
