...
//            ));
    ...
    <!-- FIN DE BUSCAR SOCIO ALERTA DE ENTREGA DE DOCUMENTOS-->
    <!--<div id="entregadocumento" class="row span12 badge_info h3"></div>-->
<!--<script>
        //   $(document).ready(function () {
        //$(function () {
        $(document).ready(function () {
              ...
              //  alert('Ingreso');
                ...
                {                    
                    if ($('#SocioMedidor_CODIGO_SOCIO').val() > 0) {                        
                        $.ajax({
                            type: "POST",
                            url: 'buscar_socio_entrega_documento',
                            data: {
                            ...
                            success: function (data) {                              
                                ...
                                if (data[0] != null) {  
                                    $('#entregadocumento').show('fast');
                                    $('#entregadocumento').html('<div class="alert alert-error flash-msg"><center><strong> ¡Atención!</strong> El usuario '+data[0].APELLIDO+' debe retirar la lista de socios del '+data[0].GRUPO+'</center></div>');
                                }
                            },
                            error: function () {
    ...
    <div class="col-xs-12">
        <div id="encabezado" class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                <div class="col-xs-4">
                    <span id="lbmensaje" class="badge label label-warning">Mensajes...</span>
                    <textarea readonly="" rows="3" style="width: 100%">
                        ...
                    </textarea>
<!--                    <button id="btn_editar_cliente" class="btn btn-info btn-sm pull-right" type='button'> <span class="glyphicon glyphicon-edit" title="Editar datos del cliente" > Editar</span> </button>
                    <button id="btn_guardar_cliente" class="btn btn-warning btn-sm pull-right" type='button'> <span class="glyphicon glyphicon-save" title="Actualizar datos del cliente"> Guardar</span> </button>-->
                </div>
            </div>
        </div>
    </div>
                                ...
                                if (data[0] != null) {
                                   $('#btnpagar').hide('fast');
                                     //$('#mensajecomunidad').html('<div class="alert alert-error flash-msg"><center><strong> ¡Atención!</strong> '+ data[1] +' tiene una deuda pendiente de '+ data[2] +' $ con la comunidad.</center></div>');
                                      $('#mensajecomunidad').html('<div class="alert alert-error flash-msg"><center><strong> ¡Atención!</strong>  tiene una deuda pendiente de '+ data[1] +' $ con la comunidad.</center></div>');
